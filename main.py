...
class App(ctk.CTk):
    def __init__(self):
        super().__init__(); self.title(APP+' — Windows Debloater & Optimizer'); self.geometry('1450x900'); self.minsize(1150,760); self.configure(fg_color=BG); self.isadmin=admin(); self.nav={}; self._page_cache={}; self._fonts={}; self._system_loading=False
        ...
        self.after(350, self.start_background_refresh)
    def font(self, size=12, weight='normal'):
        key=(size,weight)
    ...
    def header(self):
        ...
        ctk.CTkLabel(h,text='⚡  LEVSCLOADES',text_color=Y,font=self.font(28,'bold')).grid(row=0,column=0,padx=(25,5),pady=(14,0),sticky='w')
        ctk.CTkLabel(h,text='Windows Debloater & Optimizer',text_color=M,font=self.font(13)).grid(row=1,column=0,padx=(57,0),pady=(0,10),sticky='w')
        ...
        ctk.CTkLabel(self.status,text='●',text_color=G if self.isadmin else Y,font=self.font(18)).grid(row=0,column=0,rowspan=2,padx=(12,5)); ctk.CTkLabel(self.status,text='SYSTEM READY' if self.isadmin else 'LIMITED ACCESS',text_color=G if self.isadmin else Y,font=self.font(12,'bold')).grid(row=0,column=1,padx=(0,15),pady=(7,0),sticky='w'); ctk.CTkLabel(self.status,text='Real actions only',text_color=M,font=self.font(10)).grid(row=1,column=1,padx=(0,15),pady=(0,7),sticky='w')
        a=ctk.CTkFrame(h,fg_color=PANEL,border_color=BORDER,border_width=1,corner_radius=14); a.grid(row=0,column=3,rowspan=2,padx=(0,25),pady=15); ctk.CTkLabel(a,text='●' if self.isadmin else '○',text_color=G if self.isadmin else Y,font=self.font(18)).grid(row=0,column=0,rowspan=2,padx=(12,6)); ctk.CTkLabel(a,text='Administrator' if self.isadmin else 'Standard User',text_color=T,font=self.font(12,'bold')).grid(row=0,column=1,padx=(0,15),pady=(7,0),sticky='w'); ctk.CTkLabel(a,text='Full Access' if self.isadmin else 'Read-only where required',text_color=M,font=self.font(10)).grid(row=1,column=1,padx=(0,15),pady=(0,7),sticky='w'); ctk.CTkFrame(h,height=2,fg_color=Y).grid(row=2,column=0,columnspan=4,sticky='ew')
    def sidebar(self):
        ...
        for title,items in groups:
            if title:ctk.CTkLabel(s,text=title,text_color=M,font=self.font(11,'bold')).grid(row=r,column=0,padx=22,pady=(18,6),sticky='w'); r+=bool(title)
            for n,ic in items:
    ...
    def footer(self):
        f=ctk.CTkFrame(self,fg_color='#080a0c',height=48,corner_radius=0); f.grid(row=2,column=0,columnspan=2,sticky='ew'); ctk.CTkLabel(f,text='⚡ LEVSCLOADES',text_color=Y,font=self.font(12,'bold')).pack(side='left',padx=24); ctk.CTkLabel(f,text='© 2026 LEVSCLOADES — All Rights Reserved.',text_color=M,font=self.font(10)).pack(side='left'); ctk.CTkLabel(f,text='Every action reports its real result.',text_color=M,font=self.font(10)).pack(side='right',padx=24)
    def clear(self):
        for w in self.content.winfo_children():
            mgr=w.winfo_manager()
            if mgr=='pack': w.pack_forget()
            elif mgr=='grid': w.grid_remove()
            elif mgr=='place': w.place_forget()
    def show(self,n):
        ...
        if key not in self._page_cache:
            ...
            for w in self.content.winfo_children():
                mgr=w.winfo_manager()
                if mgr=='pack': saved.append((w,'pack',w.pack_info()))
                elif mgr=='grid': saved.append((w,'grid',w.grid_info()))
                elif mgr=='place': saved.append((w,'place',w.place_info()))
            self._page_cache[key]=saved
        else:
            self.clear()
            for w,mgr,info in self._page_cache[key]:
                if not w.winfo_exists(): continue
                if mgr=='pack': w.pack(**info)
                elif mgr=='grid': w.grid(**info)
                elif mgr=='place': w.place(**info)

    def page_title(self,t,sub=''):
        ctk.CTkLabel(self.content,text=t,text_color=T,font=self.font(27,'bold')).pack(anchor='w',padx=30,pady=(25,2)); ctk.CTkLabel(self.content,text=sub,text_color=M,font=self.font(12)).pack(anchor='w',padx=30,pady=(0,18)) if sub else None
    ...
    def card(self,parent,t,d=''):
        f=ctk.CTkFrame(parent,fg_color=PANEL,border_color=BORDER,border_width=1,corner_radius=14); ctk.CTkLabel(f,text=t,text_color=Y,font=self.font(15,'bold')).pack(anchor='w',padx=18,pady=(15,2)); ctk.CTkLabel(f,text=d,text_color=M,wraplength=360,justify='left').pack(anchor='w',padx=18,pady=(0,12)) if d else None; return f
    def p_dashboard(self):
        ...
        b=ctk.CTkFrame(self.content,fg_color=PANEL,border_color=BORDER,border_width=1,corner_radius=16); b.pack(fill='x',padx=30,pady=(0,18)); ctk.CTkLabel(b,text='CLEANER SYSTEM\nBETTER PERFORMANCE\nSMOOTHER EXPERIENCE',text_color=Y,font=self.font(14,'bold'),justify='right').pack(side='right',padx=28,pady=22); ctk.CTkLabel(b,text='No fake actions.\nEvery operation is verified and logged.',text_color=T,font=self.font(17,'bold'),justify='left').pack(anchor='w',padx=25,pady=22)
        ...
        info=ctk.CTkFrame(self.content,fg_color=PANEL,border_color=BORDER,border_width=1,corner_radius=14); info.pack(fill='x',padx=30,pady=18); ctk.CTkLabel(info,text='SYSTEM INFORMATION',text_color=Y,font=self.font(13,'bold')).pack(anchor='w',padx=18,pady=(14,8)); grid=ctk.CTkFrame(info,fg_color='transparent'); grid.pack(fill='x',padx=18,pady=(0,16)); self.labels={}
        for i,k in enumerate(['os','cpu','ram','gpu']):
            grid.grid_columnconfigure(i,weight=1); q=ctk.CTkFrame(grid,fg_color=PANEL2,corner_radius=10); q.grid(row=0,column=i,padx=4,sticky='nsew'); ctk.CTkLabel(q,text=k.upper(),text_color=M,font=self.font(9,'bold')).pack(anchor='w',padx=12,pady=(10,1)); l=ctk.CTkLabel(q,text='Detecting...',text_color=T,font=self.font(11,'bold'),wraplength=230,justify='left'); l.pack(anchor='w',padx=12,pady=(0,10)); self.labels[k]=l
    ...
    def start_background_refresh(self):
        if self._system_loading: return
